...
export default function CajaChica() {
  ...
  const cargar = useCallback(async () => {
    ...
    const res = await api.get(`/caja-chica/lista?${params}`);
    if (res.ok) {
    ...
    <div className="page">
      ...
      {!loading && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 12, marginBottom: 20 }}>
          ...
          <StatCard
            ...
            label="Saldo total" value={FMT(totalSaldoActual)} color="#17a2b8" />
          <StatCard
            ...
            label="Capital inicial" value={FMT(totalSaldoInicial)} color="#6c757d" />
          <StatCard
      ...
      {!loading && (
        <TableCard scrollY
          ...
          emptyText="No hay cajas chicas registradas"
          page={page}
          hasNext={hasNext}
        ...
        >
          <table>
            <thead>
              <tr>
                <th>Fecha</th><th>Estado</th><th>Monto inicial</th>
                <th>Monto actual</th><th>Gastado</th><th>Usuario</th>
              ...
              {lista.map(caja => {
                ...
                return (
                  <tr key={caja.id}>
                    ...
                    <td style={{ fontWeight: 600, color: parseFloat(caja.monto_actual) < 10 ? '#dc3545' : '#28a745' }}>
                      {FMT(caja.monto_actual)}
                    </td>
                    <td style={{ color: gastado > 0 ? '#e67e22' : '#6c757d' }}>

...
export default class FacturaControlador extends FacturaEntradaPuerto {
    ...
    async crear(req, res) {
        const datos = {...req.body, usuarioId: req.usuario.id ? req.usuario.id : null };
        const respuesta = await this.commandUC.crear(datos);

        if (respuesta.estado === 'ok') {
            const factura = respuesta.resultado;
            const abonado = parseFloat(factura.abonado ? factura.abonado : 0);
            const total = parseFloat(factura.total ? factura.total : 0);
            const metodoPago = (req.body.metodoPago ? req.body.metodoPago : '').toLowerCase();
    ...
    async editar(req, res) {
        const datos = {...req.body, usuarioId: req.usuario ? req.usuario.id : null };
    ...
    async listaGeneral(req, res) {
        const { buscar, estado, tipo, fechaDesde, fechaHasta, limit, offset } = req.query;
        const respuesta = await this.queryUC.listaGeneral({
            buscar,
            estado,
            tipo,
            fechaDesde,

export default class InventarioControlador {
  ...
  contexto(req) { return { usuarioId: req.usuario?.id, usuarioNombre: req.usuario ? `${req.usuario.nombre || ''} ${req.usuario.apellido || ''}`.trim() : null, sucursalId: req.sucursalScope?.sucursalId ?? null, sucursalNombre: req.sucursalScope?.sucursalNombre ?? null, traceId: req.traceId }; }

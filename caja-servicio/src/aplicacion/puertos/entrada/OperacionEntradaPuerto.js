// caja-servicio/src/aplicacion/puertos/entrada/OperacionEntradaPuerto.js
// caja-servicio/src/aplicacion/puertos/entrada/OperacionEntradaPuerto.js
export default class OperacionEntradaPuerto {
  procesarVenta(params) {
    throw new Error('Not implemented');
  }

  procesarCobro(params) {
    throw new Error('Not implemented');
  }

  procesarAnulacion(params) {
    throw new Error('Not implemented');
  }

  procesarAcreditacionTarjeta(params) {
    throw new Error('Not implemented');
  }

  procesarCompra(params) {
    throw new Error('Not implemented');
  }

  crearCuentaPagar(params) {
    throw new Error('Not implemented');
  }

  procesarPagoProveedor(params) {
    throw new Error('Not implemented');
  }

  procesarAnulacionCompra(params) {
    throw new Error('Not implemented');
  }

  procesarAjuste(params) {
    throw new Error('Not implemented');
  }
}

...
export default function VerCajaChica() {
  ...
  async function cerrarCaja() {
    ...
    if (res.ok) {
      ...
      Swal.fire({ title: 'Error', text: 'No se pudo cerrar la caja', icon: 'error', timer: 3000, toast: true, position: 'top-end', showConfirmButton: false });
  ...
  if (loading) return (
    <div className="page"><div className="spinner-wrapper"><div className="spinner" /><span style={{ marginLeft: 10, color: 'var(--text-muted)', fontSize: 14 }}>Cargando caja...</span></div></div>
  );
    ...
    <div className="page">
      ...
      <div className="page-header" style={{ marginBottom: 20 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            <h1 className="page-title" style={{ margin: 0 }}>Caja Chica — {FECHA(caja.fecha)}</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 12, marginBottom: 20 }}>
        <StatCard icon={<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect width="20" height="14" x="2" y="5" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/></svg>}
          label="Saldo inicial" value={FMT(caja.monto_inicial)} color="#3498db" subtext={`${resumen.cantidad_movimientos || 0} mov. total`} />
        ...
        <StatCard icon={<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>}
          label="Saldo actual" value={FMT(caja.monto_actual)}
          color={parseFloat(caja.monto_actual) >= 10 ? '#27ae60' : '#e74c3c'} />
      </div>

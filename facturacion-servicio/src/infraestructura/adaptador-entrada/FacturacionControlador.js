export default class FacturacionControlador {
  constructor(usesCases) { this.usesCases = usesCases; }
  contexto(req) { return { usuarioId: req.usuario?.id, usuarioNombre: req.usuario ? `${req.usuario.nombre || ''} ${req.usuario.apellido || ''}`.trim() : null, sucursalId: req.sucursalScope?.sucursalId ?? null, sucursalNombre: req.sucursalScope?.sucursalNombre ?? null, authorization: req.headers.authorization, traceId: req.traceId }; }
  async lista(req, res) { const r = await this.usesCases.listar(req.params.recurso, { ...req.query, sucursalId: req.sucursalScope?.filtroLectura ?? null }); res.status(r.estado === 'ok' ? 200 : 400).json({ ...r, traceId: req.traceId }); }

...
export default class FacturaControlador extends FacturaEntradaPuerto {
  ...
  contexto(req) { return { usuarioId: req.usuario?.id ?? null, usuarioNombre: req.usuario ? `${req.usuario.nombre || ''} ${req.usuario.apellido || ''}`.trim() : null, sucursalId: req.usuario?.sucursalId ?? null, authorization: req.headers.authorization, traceId: req.traceId, motivo: req.body?.motivo }; }

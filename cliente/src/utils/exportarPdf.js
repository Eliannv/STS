...
export function exportarKardexPdf({ product, productFields, indicators, indicatorDefinitions, filters, filterDefinitions, columns, rows, generatedAt }) {
  ...
  const productData = productFields.map(field => ({ label: field.label, value: displayValue(product?.[field.key], field.type) }));
  ...
  popup.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Kardex ${escapeHtml(product?.codigo || '')}</title><style>
    ...
    <h1>Kardex por producto</h1><div class="generated">Generado: ${escapeHtml(new Date(generatedAt).toLocaleString('es-EC'))}</div>
    <h2>Ficha del producto</h2><div class="grid">${rowsHtml(productData)}</div>
    <h2>Indicadores</h2><div class="grid">${rowsHtml(summaryData)}</div>
    <h2>Filtros utilizados</h2><div class="grid">${rowsHtml(filterData)}</div>
    <h2>Movimientos</h2>${rows.length ? `<table><thead><tr>${tableHead}</tr></thead><tbody>${tableBody}</tbody></table>` : '<div class="empty">Este producto aún no registra movimientos de inventario.</div>'}
  </body></html>`);

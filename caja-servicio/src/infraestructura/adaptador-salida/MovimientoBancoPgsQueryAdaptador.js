// caja-servicio/src/infraestructura/adaptador-salida/MovimientoBancoPgsQueryAdaptador.js
...
const construirWhere = (cajaId, filtro) => {
  const where = { caja_banco_id: cajaId };
...
export default class MovimientoBancoPgsQueryAdaptador extends MovimientoSalidaQueryPuerto {
  async findByCaja(cajaId, filtro) {
    ...
    const resultado = await ModeloMovimientoBanco.findAndCountAll({
  ...
  async findByIdempotencyKey(key) {
    return mapearEntidad(
      await ModeloMovimientoBanco.findOne({
  ...
  async findById(id) {
    return mapearEntidad(await ModeloMovimientoBanco.findByPk(id));
  }

// caja-servicio/src/infraestructura/modelos/ModeloCajaBanco.js
import { DataTypes } from 'sequelize';
import sequelize from '../base-dato/Postgresql.js';

const ModeloCajaBanco = sequelize.define(
  'CajaBanco',
  {
    id: {
      type: DataTypes.INTEGER,
      primaryKey: true,
      autoIncrement: true,
    },
    fecha: {
      type: DataTypes.DATEONLY,
      allowNull: false,
    },
    saldo_inicial: {
      type: DataTypes.DECIMAL(14, 2),
      allowNull: false,
      defaultValue: 0,
    },
    saldo_actual: {
      type: DataTypes.DECIMAL(14, 2),
      allowNull: false,
      defaultValue: 0,
    },
    estado: {
      type: DataTypes.ENUM('ABIERTA', 'CERRADA'),
      allowNull: false,
      defaultValue: 'ABIERTA',
    },
    usuario_id: DataTypes.INTEGER,
    usuario_nombre: DataTypes.STRING(150),
    observacion: DataTypes.TEXT,
    activo: {
      type: DataTypes.BOOLEAN,
      allowNull: false,
      defaultValue: true,
    },
    created_at: DataTypes.DATE,
    updated_at: DataTypes.DATE,
    cerrado_en: DataTypes.DATE,
    cerrado_por_id: DataTypes.INTEGER,
    cerrado_por_nombre: DataTypes.STRING(150),
    periodo: {
      type: DataTypes.DATEONLY,
      allowNull: false,
      unique: 'uq_cajas_banco_periodo',
    },
    fecha_apertura: {
      type: DataTypes.DATE,
      allowNull: false,
      defaultValue: DataTypes.NOW,
    },
    ingresos_acumulados: {
      type: DataTypes.DECIMAL(14, 2),
      allowNull: false,
      defaultValue: 0,
    },
    egresos_acumulados: {
      type: DataTypes.DECIMAL(14, 2),
      allowNull: false,
      defaultValue: 0,
    },
    total_movimientos: {
      type: DataTypes.INTEGER,
      allowNull: false,
      defaultValue: 0,
    },
    saldo_contado_cierre: DataTypes.DECIMAL(14, 2),
    diferencia_cierre: DataTypes.DECIMAL(14, 2),
    motivo_diferencia: DataTypes.TEXT,
    observacion_cierre: DataTypes.TEXT,
    permite_saldo_negativo: {
      type: DataTypes.BOOLEAN,
      allowNull: false,
      defaultValue: false,
    },
  },
  {
    tableName: 'cajas_banco',
    timestamps: false,
    freezeTableName: true,
  },
);

export default ModeloCajaBanco;

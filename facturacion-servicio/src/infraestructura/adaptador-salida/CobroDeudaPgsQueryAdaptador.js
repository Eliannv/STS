...
export default class CobroDeudaPgsQueryAdaptador extends CobroDeudaQuerySalidaPuerto {
  ...
  async abonosPorFactura(facturaId, sucursalId = null) {
    const where = { factura_id: facturaId, ...(sucursalId ? { sucursal_id: Number(sucursalId) } : {}) };
    return { estado: 'ok', resultado: await Deuda.findAll({ where, order: [['fecha_pago', 'DESC']] }) };
  }
  ...
  // El catálogo de clientes es global, pero su actividad se ve por sucursal:
  // un operador solo suma las facturas y cobros hechos en la suya.
  async resumenClienteDeuda(clienteId, sucursalId = null) {
    const alcance = sucursalId ? { sucursal_id: Number(sucursalId) } : {};
    const facturas = await Factura.findAll({ where: { cliente_id: clienteId, ...alcance } });
    const abonos = await Deuda.findAll({ where: { cliente_id: clienteId, ...alcance } });
  ...
  async deudasPaginadas(offset = 0, limite = 5, sucursalId = null) {
    const where = { estado_pago: 'PENDIENTE', saldo_pendiente: { [Op.gt]: 0 }, ...(sucursalId ? { sucursal_id: Number(sucursalId) } : {}) };

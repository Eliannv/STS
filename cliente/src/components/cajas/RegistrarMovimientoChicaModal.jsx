...
export default function RegistrarMovimientoChicaModal({ abierto, cajaChicaId, saldoActual = 0, onCerrar, onRegistrado }) {
  const [form, setForm]     = useState({ tipo: 'EGRESO', descripcion: '', monto: '', fecha: HOY_HORA(), referencia: '' });
  const [error, setError]   = useState('');
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (abierto) {
      setForm({ tipo: 'EGRESO', descripcion: '', monto: '', fecha: HOY_HORA(), referencia: '' });
      setError('');
    }
  }, [abierto]);
  ...
  const saldoTras = esEgreso
    ? Math.max(0, saldoActual - montoVal)
    : saldoActual + montoVal;
    ...
    <>
      <div style={{ background: '#fff', border: '1px solid var(--border-color)', borderRadius: 10, padding: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: 13 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <span style={{ fontWeight: 600 }}>Movimiento Caja Chica</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
      ...
      <div style={{ background: '#fff', border: '1px solid var(--border-color)', borderRadius: 10, padding: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 12 }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#27ae60" strokeWidth="2"><circle cx="12" cy="12" r="2"/><rect width="18" height="12" x="3" y="6" rx="2"/></svg>
          <span style={{ fontWeight: 700, fontSize: 13 }}>Resumen</span>
        </div>
        {[
          ['Concepto', 'Descripción del gasto'],
          ['Monto', 'Valor a registrar'],
          ['Referencia', 'Factura o comprobante'],
        ].map(([k, v]) => (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <input
            ...
            placeholder="Ej: Compra de suministros, pago de servicio..."
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <input
            ...
            placeholder="Nro. factura, recibo, etc."
            onChange={handleChange}
            style={{ padding: '8px 12px', border: '1px solid var(--border-color)', borderRadius: 8, fontSize: 14 }}
          />
        </div>

...
export default class SucursalControlador extends SucursalEntradaPuerto {
  ...
  async lista(req, res) { const r = await this.usesCases.lista({ incluirInactivas: req.query.incluirInactivas === 'true' }); return res.json({ ...r, traceId: req.traceId }); }
  ...
  async editar(req, res) { const r = await this.usesCases.editar(new SucursalDTO({ ...req.body, id: Number(req.params.id) })); return res.status(r.estado === 'ok' ? 200 : 400).json({ ...r, traceId: req.traceId }); }
  async eliminar(req, res) { const r = await this.usesCases.eliminar(Number(req.params.id)); return res.status(r.estado === 'ok' ? 200 : 400).json({ ...r, traceId: req.traceId }); }
}

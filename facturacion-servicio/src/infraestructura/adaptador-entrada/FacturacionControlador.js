export default class FacturacionControlador {
  ...
  contexto(req) { return { usuarioId: req.usuario?.id, usuarioNombre: req.usuario ? `${req.usuario.nombre || ''} ${req.usuario.apellido || ''}`.trim() : null, sucursalId: req.usuario?.sucursalId, authorization: req.headers.authorization, traceId: req.traceId }; }

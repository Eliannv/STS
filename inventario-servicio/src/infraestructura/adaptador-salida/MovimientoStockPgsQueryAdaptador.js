...
export default class MovimientoStockPgsQueryAdaptador extends MovimientoStockSalidaQueryPuerto {
  async listar({ productoId, codigo, sucursalId, naturaleza, tipoMovimiento, origen, referenciaTipo, referenciaId, fechaDesde, fechaHasta, limit = 100, offset = 0 } = {}) {
    const where = {};
    ...
    const resultado = await MovimientoStock.findAll({ where, order: [['fecha_operacion', 'DESC'], ['id', 'DESC']], limit: Math.min(Number(limit) || 100, 500), offset: Math.max(Number(offset) || 0, 0) });

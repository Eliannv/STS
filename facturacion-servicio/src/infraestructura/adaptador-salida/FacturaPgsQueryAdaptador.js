...
const conDetalles = async (factura) => ({ ...factura.toJSON(), detalles: await DetalleFactura.findAll({ where: { factura_id: factura.id }, order: [['id', 'ASC']] }) });

export default class FacturaPgsQueryAdaptador extends FacturaSalidaQueryPuerto {
  async listaPorCliente(clienteId) {
    const facturas = await Factura.findAll({ where: { cliente_id: clienteId, deleted_at: null }, order: [['created_at', 'DESC']] });
  ...
  async listaGeneral({ buscar, estado, tipo, fechaDesde, fechaHasta, limit = 15, offset = 0 } = {}) {
    ...
    if (fechaDesde || fechaHasta) where.created_at = { ...(fechaDesde ? { [Op.gte]: fechaDesde } : {}), ...(fechaHasta ? { [Op.lte]: fechaHasta } : {}) };
    const facturas = await Factura.findAll({ where, order: [['created_at', 'DESC']], limit: Math.min(Number(limit) || 15, 100), offset: Math.max(Number(offset) || 0, 0) });

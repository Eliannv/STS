// inventario-servicio/src/infraestructura/modelos/Modelos.js
import { DataTypes } from 'sequelize';
import sequelize from '../base-dato/Postgresql.js';
import Egreso from './ModeloEgresoMercaderia.js';
import DetalleEgreso from './ModeloDetalleEgreso.js';
...
const Ingreso = sequelize.define('Ingreso', { id: { type: DataTypes.INTEGER, primaryKey: true, autoIncrement: true }, id_personalizado: DataTypes.STRING(10), proveedor_id: DataTypes.INTEGER, proveedor_nombre: DataTypes.STRING(150), numero_factura: { type: DataTypes.STRING(50), allowNull: false }, fecha: { type: DataTypes.DATEONLY, allowNull: false }, tipo_compra: DataTypes.ENUM('CONTADO','CREDITO'), metodo_pago: DataTypes.STRING(30), caja_tipo: DataTypes.STRING(10), caja_id: DataTypes.INTEGER, fecha_vencimiento: DataTypes.DATEONLY, estado_financiero: DataTypes.STRING(20), cuenta_pagar_id: DataTypes.INTEGER, observacion: DataTypes.TEXT, descuento: DataTypes.DECIMAL(14,2), flete: DataTypes.DECIMAL(14,2), iva: DataTypes.DECIMAL(14,2), total: DataTypes.DECIMAL(14,2), estado: DataTypes.ENUM('BORRADOR','FINALIZADO','ANULADO'), usuario_id: DataTypes.INTEGER, sucursal_id: DataTypes.INTEGER, sucursal_nombre: DataTypes.STRING(100), created_at: DataTypes.DATE, updated_at: DataTypes.DATE }, { tableName: 'ingresos', ...common });
...
const MovimientoStock = sequelize.define('MovimientoStock', {
  ...
  tipo_movimiento: { type: DataTypes.ENUM('INVENTARIO_INICIAL','COMPRA','VENTA','DEVOLUCION_CLIENTE','DEVOLUCION_PROVEEDOR','EGRESO','AJUSTE','TRANSFERENCIA_ENTRADA','TRANSFERENCIA_SALIDA','ANULACION_VENTA','ANULACION_COMPRA','ANULACION_EGRESO','REVALORIZACION','COMPENSACION','MERMA','ROTURA','ROBO','PERDIDA','VENCIMIENTO','CONSUMO_INTERNO','MUESTRA','DONACION','OBSOLESCENCIA','RETIRO_CALIDAD'), allowNull: false },

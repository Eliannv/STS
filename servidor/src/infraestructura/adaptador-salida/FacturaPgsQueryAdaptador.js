...
export default class FacturaPgsQueryAdaptador extends FacturaSalidaQueryPuerto {
  ...
  async listaGeneral({ buscar, estado, tipo, fechaDesde, fechaHasta } = {}) {
    try {
      ...
      if (estado)     { params.push(estado);     where.push(`f.estado_pago = $${params.length}`); }
      if (tipo)       { params.push(tipo);       where.push(`f.tipo_venta = $${params.length}`); }
      if (fechaDesde) { params.push(fechaDesde); where.push(`f.created_at::date >= $${params.length}`); }
      if (fechaHasta) { params.push(fechaHasta); where.push(`f.created_at::date <= $${params.length}`); }
      const sql = `
        SELECT f.*,
               f.estado_pago  AS estado,
               f.tipo_venta   AS tipo,
